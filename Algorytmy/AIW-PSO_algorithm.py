...
def generate_flows(G, K):
    ...
    while len(flows) < K:
        ...
        intensity = np.random.randint(1, 10)   # losowa intensywność 1–10
...
def compute_load(weights, flows):
    ...
    for (s, d, intensity) in flows:
        ...
        for i in range(len(path) - 1):
            e = (path[i], path[i+1])
            idx = edge_to_idx[e]
            load[idx] += intensity   # zamiast +1 dodajemy wartość flowu
...
# ---------------------------------------------------------
# 4. Definicja problemu optymalizacyjnego
# ---------------------------------------------------------
problem = {
...
# ---------------------------------------------------------
# 6. Obliczenia dodatkowych metryk
# ---------------------------------------------------------
...
print("Min Fitness:", g_best.target.fitness)
